...
export function getFirstDate({year, month}: Page): Date {
	return new Date(year, monthToNumber(month), 1)
}

function getLastDate({year, month}: Page): Date {
	const endDate = getFirstDate(nextPage({year, month}))
...
export const getAllDatesOnPage = (page: Page, calendarStartDay: CalendarStartDay): { underflow: Date[], month: Date[], overflow: Date[] } => {
	const startDate = getFirstDate(page)

	const endDate = getLastDate(page)
