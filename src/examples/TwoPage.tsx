...
import { getStartDate, nextPage } from "../helpers";
import { usePage } from "../usePage"

const pink = '#00c67e';
const pinkTint = '#f2fffa';
...
const WeekdayTitles = () => (
    <div style={{ gridTemplateColumns: 'repeat(7, 1fr)', display: 'grid', color: 'grey', gridGap: 4, textAlign: 'center', marginBottom: 5 }}>
...
export const DateInput = ({value, selected, onFocus, disabled}: {disabled?: boolean, value: string, selected: boolean, onFocus: () => void}) => {
    return <input readOnly value={value} onFocus={onFocus} onChange={() => undefined} style={{ borderWidth: selected ? 3 : 1}} disabled={disabled} />
}
...
export const TwoPage = () => {
    ...
    const canGoBack = useMemo(() => {
        const pageStartDate = getStartDate(page);
        if (range === undefined) {
...
const FormWithPanel = ({ fields, panel, panelHeader }: { fields: ReactNode, panel: ReactNode, panelHeader: ReactNode }) => (
    <div style={{ display: 'flex', flexDirection: 'row', fontFamily: 'sans-serif', margin: 35, borderRadius: 5, boxShadow: "0 0 20px 20px #0000001a", padding: 15,  width: 'fit-content' }}>
